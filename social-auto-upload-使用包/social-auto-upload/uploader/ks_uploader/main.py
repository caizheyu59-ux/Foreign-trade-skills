        ...
        try:
            ...
            while publish_attempt < max_publish_attempts:
                try:
                    ...
                    result = {"success": True, "status": "published", "message": "视频发布成功", "url": page.url}
                    break
                except Exception as exc:
            ...
            if not result["success"]:
                result = {"success": False, "status": "publish_timeout", "message": f"发布超时（尝试 {max_publish_attempts} 次）", "url": page.url}
                kuaishou_logger.error(_msg("😵", result["message"]))
                if self.debug:
                    await page.screenshot(full_page=True)
...
class KSNote(KSBaseUploader):
    ...
    async def upload_note_content(self, page: Page) -> None:
        ...
        kuaishou_logger.error(_msg("😵", self._result["message"]))
        if self.debug:
            await page.screenshot(full_page=True)
