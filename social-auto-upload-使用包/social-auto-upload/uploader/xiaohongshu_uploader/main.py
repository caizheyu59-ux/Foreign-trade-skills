...
class XiaoHongShuBaseUploader(BaseVideoUploader):
    ...
    async def fill_desc(self, page: Page) -> None:
        ...
        # Try multiple selectors for the description/editor area
        desc_locators = [
        ...
        desc = None
        for sel in desc_locators:
            if await page.locator(sel).count() > 0:
                desc = page.locator(sel)
                break
        
        if not desc:
            xiaohongshu_logger.warning(_msg("😵", "未找到描述输入框，跳过描述填写"))
            return
        ...
        is_editable = await desc.evaluate('el => el.isContentEditable')
        if is_editable:
            await desc.fill("")
        else:
    ...
    async def fill_tags(self, page: Page) -> None:
        ...
        if not getattr(self, "desc", ""):
            ...
            if await desc.count():
                ...
                desc_ce = page.locator('div[contenteditable="true"]').first
                if await desc_ce.count():
                    await desc_ce.click()
        ...
        for tag in self.tags:  # 循环处理所有 tags
            ...
            # Topic suggestion dropdown uses Tippy.js tooltips
            # Structure: .tippy-box > .tippy-content > .items > .item
            ...
            topic_item = None
            for sel in topic_item_selectors:
                try:
                    count = await page.locator(sel).count()
                    if count > 0:
                        first_el = page.locator(sel).first
                        is_visible = await first_el.is_visible()
                        if is_visible:
                            topic_item = first_el
                            xiaohongshu_logger.debug(_msg("🧍", f"话题选择器命中: {sel}"))
                            break
                except Exception:
                    continue
            ...
            if topic_item:
                ...
                xiaohongshu_logger.warning(_msg("😵", "未找到话题选择器，尝试 Enter 键选择"))
...
class XiaoHongShuVideo(XiaoHongShuBaseUploader):
    ...
    async def upload_video_content(self, page: Page) -> None:
        ...
        self._result = {"success": False, "status": "unknown", "message": "", "url": ""}
        ...
        while publish_attempt < max_publish_attempts:
            try:
                ...
                self._result = {"success": True, "status": "published", "message": "视频发布成功", "url": page.url}
                return
            except Exception:
        ...
        self._result = {"success": False, "status": "publish_timeout", "message": f"发布超时（尝试 {max_publish_attempts} 次）", "url": page.url}
        xiaohongshu_logger.error(_msg("😵", self._result["message"]))
        if self.debug:
            await page.screenshot(full_page=True)
    ...
    async def upload(self, playwright: Playwright) -> dict:
        ...
        result = {"success": False, "status": "unknown", "message": "", "url": ""}
        try:
            page = await context.new_page()
    ...
    async def upload_note_content(self, page: Page) -> None:
        ...
        while publish_attempt < max_publish_attempts:
            try:
                ...
                self._result = {"success": True, "status": "published", "message": "图文发布成功", "url": page.url}
                return
            except Exception:
        ...
        self._result = {"success": False, "status": "publish_timeout", "message": f"图文发布超时（尝试 {max_publish_attempts} 次）", "url": page.url}
        xiaohongshu_logger.error(_msg("😵", self._result["message"]))
        if self.debug:
            await page.screenshot(full_page=True)
