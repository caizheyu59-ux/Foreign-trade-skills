    ...
    async def upload(self, playwright: Playwright) -> None:
        ...
        while publish_attempt < max_publish_attempts:
            try:
                ...
                result = {"success": True, "status": "published", "message": "视频发布成功", "url": current_url}
                break
            except Exception:
        ...
        if not result["success"]:
            ...
            douyin_logger.error(_msg("😵", result["message"]))
            if self.debug:
                await page.screenshot(full_page=True)
    ...
    async def upload_note_content(self, page: Page) -> None:
        ...
        while True:
            try:
                ...
                return {"success": True, "status": "published", "message": "图文发布成功", "url": page.url}
            except Exception:
                douyin_logger.info(_msg("🏃", "小人正在冲刺发布图文"))
                await asyncio.sleep(0.5)
        return {"success": False, "status": "publish_failed", "message": "图文发布超时", "url": ""}
