...
--title "示例标题"
--desc "示例简介"
--desc-file path/to/desc.txt   # 从文件读取描述（避免 shell 引号问题）
--tags 运动,训练
...
--title "图文标题"
--note "图文内容"
--note-file path/to/note.txt   # 从文件读取正文（避免 shell 引号问题）
--tags 图文,测试
```

## 避免 Shell 引号问题

当描述或正文内容中包含双引号（如 `"新建小部件"`）时，PowerShell 的参数解析会崩溃。
所有平台都支持 `--desc-file`（视频）和 `--note-file`（图文）参数，从 UTF-8 文件读取内容：

```bash
# 视频上传 - 描述从文件读取
sau bilibili upload-video --account kingsway --file video.mp4 --title "标题" --desc-file desc.txt --tid 249
...
**优先级**：`--desc-file` / `--note-file` 优先于 `--desc` / `--note`，两者都传时以文件内容为准。
