...
from uploader.kingsway_uploader.main import (
    KingswayVideoUploadRequest,
    check_kingsway_config,
    setup_kingsway_config,
    upload_kingsway_video,
)
...
@dataclass(slots=True)
class KingswayVideoUploadRequestCLI:
    account_name: str
    video_file: Path
    title: str
    description: str
...
async def upload_video(request: DouyinVideoUploadRequest) -> dict:
...
async def upload_note(request: DouyinNoteUploadRequest) -> dict:
    account_file = resolve_account_file("douyin", request.account_name)
    is_ready = await douyin_setup(str(account_file), handle=False)
    if not is_ready:
        raise RuntimeError(
...
async def upload_kuaishou_video(request: KuaishouVideoUploadRequest) -> dict:
...
async def upload_bilibili_video(request: BilibiliVideoUploadRequest) -> dict:
    ...
    if result.returncode != 0:
        return {"success": False, "status": "upload_failed", "message": (result.stderr or result.stdout or "").strip() or "Bilibili upload failed", "url": ""}
    return {"success": True, "status": "uploaded", "message": "视频已上传到 Bilibili", "url": ""}


def _print_upload_result(platform: str, content_type: str, result: dict) -> None:
    """Print structured upload result to stdout."""
    ...
    if success:
        print(f"✅ {platform} {content_type} 发布成功！")
        print(f"   状态: {status}")
        print(f"   消息: {message}")
        if url:
            print(f"   链接: {url}")
    else:
        print(f"❌ {platform} {content_type} 发布失败")
        print(f"   状态: {status}")
        print(f"   消息: {message}")
        if url:
            print(f"   当前页面: {url}")
...
def read_text_file(value: str) -> str:
    """Read text content from a file. Used to avoid shell quoting issues with --desc/--note."""
    path = Path(value)
    if not path.is_file():
        raise argparse.ArgumentTypeError(f"File not found: {value}")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    upload_video_parser.add_argument("--desc-file", type=read_text_file, default=None, help="Read description from file (avoids shell quoting issues)")
    ...
    upload_note_parser.add_argument("--note-file", type=read_text_file, default=None, help="Read note content from file (avoids shell quoting issues)")
    ...
    kuaishou_upload_video_parser.add_argument("--desc-file", type=read_text_file, default=None, help="Read description from file (avoids shell quoting issues)")
    ...
    kuaishou_upload_note_parser.add_argument("--note-file", type=read_text_file, default=None, help="Read note content from file (avoids shell quoting issues)")
    ...
    xiaohongshu_upload_video_parser.add_argument("--desc-file", type=read_text_file, default=None, help="Read description from file (avoids shell quoting issues)")
    ...
    xiaohongshu_upload_note_parser.add_argument("--note-file", type=read_text_file, default=None, help="Read note content from file (avoids shell quoting issues)")
    ...
    bilibili_upload_video_parser.add_argument("--desc", default="", help="Video description")
    bilibili_upload_video_parser.add_argument("--desc-file", type=read_text_file, default=None, help="Read description from file (avoids shell quoting issues)")
    ...
    # --- Kingsway ---
    kingsway_parser = platform_parsers.add_parser("kingsway", help="Kingsway Video operations (API-based)")
    kingsway_actions = kingsway_parser.add_subparsers(dest="action", required=True)

    kingsway_check_parser = kingsway_actions.add_parser("check", help="Check Kingsway API key config")
    kingsway_check_parser.add_argument("--account", required=True, help="Kingsway account name")

    kingsway_setup_parser = kingsway_actions.add_parser("setup", help="Set up Kingsway API key")
    kingsway_setup_parser.add_argument("--account", required=True, help="Kingsway account name")
    kingsway_setup_parser.add_argument("--api-key", required=True, help="Kingsway API key (sk-...)")
    kingsway_setup_parser.add_argument("--base-url", default="", help="Optional API base URL")

    kingsway_upload_video_parser = kingsway_actions.add_parser("upload-video", help="Upload one video to Kingsway")
    kingsway_upload_video_parser.add_argument("--account", required=True, help="Kingsway account name")
    kingsway_upload_video_parser.add_argument("--file", required=True, type=existing_file_path, help="Video file path")
    kingsway_upload_video_parser.add_argument("--title", required=True, help="Video title")
    kingsway_upload_video_parser.add_argument("--desc", default="", help="Video description")
    kingsway_upload_video_parser.add_argument("--desc-file", type=read_text_file, default=None, help="Read description from file (avoids shell quoting issues)")
    kingsway_upload_video_parser.add_argument("--tags", default="", help="Comma-separated tags, such as tag1,tag2")
    kingsway_upload_video_parser.add_argument("--lang", default="en", help="SEO page language (default: en)")
...
async def dispatch(args: argparse.Namespace) -> int:
    if args.platform == "douyin":
        ...
        if args.action == "upload-video":
            request = DouyinVideoUploadRequest(
                ...
                title=args.title,
                description=getattr(args, "desc_file", None) or args.desc,
            ...
            result = await upload_video(request)
            _print_upload_result("Douyin", "video", result)
        ...
        if args.action == "upload-note":
            request = DouyinNoteUploadRequest(
                ...
                title=args.title,
                note=getattr(args, "note_file", None) or args.note,
                tags=parse_tags(args.tags),
                publish_date=args.schedule or 0,
                publish_strategy=publish_strategy,
                debug=args.debug,
            ...
            result = await upload_note(request)
            _print_upload_result("Douyin", "note", result)
            return 0 if result.get("success") else 1
    ...
    if args.platform == "kuaishou":
        ...
        if args.action == "upload-video":
            ...
            result = await upload_kuaishou_video(request)
            _print_upload_result("Kuaishou", "video", result)
            return 0 if result.get("success") else 1
        ...
        if args.action == "upload-note":
            ...
            _print_upload_result("Kuaishou", "note", result)
            return 0 if result.get("success") else 1
    ...
    if args.platform == "xiaohongshu":
        ...
        if args.action == "upload-video":
            ...
            _print_upload_result("Xiaohongshu", "video", result)
            return 0 if result.get("success") else 1
        ...
        if args.action == "upload-note":
            ...
            _print_upload_result("Xiaohongshu", "note", result)
            return 0 if result.get("success") else 1
    ...
    if args.platform == "bilibili":
        ...
        if args.action == "upload-video":
            ...
            result = await upload_bilibili_video(request)
            _print_upload_result("Bilibili", "video", result)
            return 0 if result.get("success") else 1

        raise RuntimeError(f"Unsupported Bilibili action: {args.action}")

    if args.platform == "kingsway":
        if args.action == "check":
            is_valid = check_kingsway_config(args.account)
            print("valid" if is_valid else "invalid")
            return 0 if is_valid else 1

        if args.action == "setup":
            setup_kingsway_config(args.account, args.api_key, args.base_url or None)
            print(f"Kingsway API key configured for account: {args.account}")
            return 0

        if args.action == "upload-video":
            request = KingswayVideoUploadRequestCLI(
                account_name=args.account,
                video_file=args.file,
                title=args.title,
                description=getattr(args, "desc_file", None) or args.desc,
            ...
            result = await upload_kingsway_video(
                KingswayVideoUploadRequest(
                    account_name=request.account_name,
                    video_file=str(request.video_file),
                    title=request.title,
            ...
            if result.get("success"):
                print(f"Kingsway video upload completed: {result.get('sourceVideoId')}")
            else:
                print(f"Kingsway video upload failed: {result}", file=sys.stderr)
                return 1
            return 0

        raise RuntimeError(f"Unsupported Kingsway action: {args.action}")
